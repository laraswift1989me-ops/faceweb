...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800/50 p-3 z-50 flex items-center justify-around pb-6 lg:hidden shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
      {navItems.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
          className={({ isActive }) => `
            ...
            ${isActive 
              ? "bg-gradient-to-br from-cyan-500/20 to-blue-500/10 text-cyan-400 scale-110 shadow-lg shadow-cyan-500/20" 
