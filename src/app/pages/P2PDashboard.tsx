...
import {
  ArrowLeftRight, Cpu, ShieldCheck, TrendingUp, Users,
...
const FEATURES = [
  {
    ...
    description: "Neural network driven order pairing for optimal execution and zero slippage.",
    cardCls: "bg-cyan-50 dark:bg-cyan-500/10 border-cyan-200 dark:border-cyan-500/30",
    iconCls: "text-cyan-500 dark:text-cyan-400",
  },
  {
    icon: ShieldCheck,
    title: "Smart Escrow",
    description: "AI-monitored escrow contracts with automated release and dispute resolution.",
    cardCls: "bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30",
    iconCls: "text-emerald-500 dark:text-emerald-400",
  },
  {
    icon: TrendingUp,
    title: "Dynamic Pricing",
    description: "Real-time market analysis drives fair rates, eliminating price manipulation.",
    cardCls: "bg-indigo-50 dark:bg-indigo-500/10 border-indigo-200 dark:border-indigo-500/30",
    iconCls: "text-indigo-500 dark:text-indigo-400",
  },
  {
    icon: Users,
    title: "KYC Reputation",
    description: "Every counterparty is verified with an AI-computed on-chain reputation score.",
    cardCls: "bg-violet-50 dark:bg-violet-500/10 border-violet-200 dark:border-violet-500/30",
    ...
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 rounded-[40px] p-8 lg:p-12 border border-slate-700/50 shadow-2xl">
        <div className="absolute top-0 right-0 w-72 h-72 opacity-[0.04] pointer-events-none">
          <ArrowLeftRight className="w-full h-full text-cyan-400" strokeWidth={0.5} />
        </div>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          ...
          <div className="grid grid-cols-3 gap-4 lg:gap-6 shrink-0">
            {[
              ...
              <div key={label} className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/50 text-center">
                <p className={`${color} text-xl font-black italic tracking-tighter`}>{value}</p>
                <p className="text-slate-400 text-[9px] font-black tracking-widest uppercase mt-1">{label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[36px] p-7 border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {FEATURES.map(({ icon: Icon, title, description, cardCls, iconCls }, i) => (
                <motion.div
                  key={title}
                ...
                >
                  <div className={`w-11 h-11 rounded-xl border flex items-center justify-center shrink-0 ${cardCls}`}>
                    <Icon className={`w-5 h-5 ${iconCls}`} />
                  </div>
                  <div>
                    <p className="text-slate-900 dark:text-white font-bold text-sm uppercase tracking-tight mb-1">{title}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-xs font-medium leading-relaxed">{description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[36px] p-7 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-slate-900 dark:text-white font-black italic uppercase tracking-tight">Development Roadmap</h3>
                <p className="text-slate-400 dark:text-slate-500 text-[10px] font-bold tracking-widest uppercase">Q2 2025 Target</p>
              </div>
            </div>
              ...
              {ROADMAP.map(({ phase, label, status, desc }) => (
                ...
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-xs font-black border ${
                    ...
                    {status === "active" ? <Zap className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`font-bold text-sm uppercase tracking-tight ${status === "active" ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}>
                      {label}
                    </p>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[36px] p-7 border border-slate-200 dark:border-slate-800">
            ...
            <h3 className="text-slate-900 dark:text-white font-black italic uppercase tracking-tighter text-lg mb-2">Early Access</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed mb-6">
              Members who reach <span className="text-cyan-500 dark:text-cyan-400 font-bold">Level 3</span> or above
              will receive priority invitations when P2P launches.
            </p>
            ...
            <div className="space-y-3">
              {[
                ...
                >
                  <div>
                    <p className={`text-sm font-bold uppercase tracking-tight ${met ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}>
                      {label}
                    </p>
                    <p className={`text-[10px] font-bold tracking-widest uppercase mt-0.5 ${met ? "text-emerald-500 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
                      {note}
                    </p>
                  </div>
                  {met && <CheckCircle2 className="w-4 h-4 text-emerald-500 dark:text-emerald-400 shrink-0" />}
                </div>
              ))}
            </div>
          ...
          {/* What is P2P */}
          <section className="bg-white dark:bg-slate-900 rounded-[36px] p-7 border border-slate-200 dark:border-slate-800">
            <h3 className="text-slate-400 dark:text-slate-500 text-[10px] font-black tracking-[0.2em] uppercase mb-5">How P2P Works</h3>
            <div className="space-y-4">
              {[
                { step: "1", text: "Post a buy or sell order at your desired rate" },
                { step: "2", text: "AI matches you with the best counterparty available" },
                { step: "3", text: "Funds locked in smart escrow during negotiation" },
                { step: "4", text: "Confirm trade — assets released automatically" },
              ].map(({ step, text }) => (
                <div key={step} className="flex items-start gap-3">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 text-sm font-medium leading-relaxed pt-1">{text}</p>
                </div>
              ))}
            </div>
          </section>
