...
export function Profile() {
  ...
  return (
    <div className="space-y-8 lg:space-y-10 animate-in fade-in duration-700">
      ...
      {/* ── HERO ─────────────────────────────────────────────────────────── */}
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800/50 to-slate-950 p-8 lg:p-10 rounded-[40px] border border-slate-700/50 shadow-2xl overflow-hidden">
        {/* bg glow blobs */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="relative shrink-0">
            <div className="w-28 h-28 rounded-[32px] bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center shadow-2xl shadow-cyan-500/20 ring-4 ring-slate-900">
              <span className="text-white font-black text-5xl italic">{userName.slice(0,1).toUpperCase()}</span>
            </div>
            <div className={`absolute -bottom-2 -right-2 w-9 h-9 rounded-xl flex items-center justify-center border-4 border-slate-900 shadow-lg ${isKycVerified ? "bg-emerald-500" : isKycPending ? "bg-amber-500" : "bg-slate-700"}`}>
          ...
          <div className="flex-1 text-center md:text-left space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase flex items-center gap-1.5 border
                bg-${ac}-500/10 border-${ac}-500/20 text-${ac}-400`}>
                <KycIcon className="w-3 h-3" /> KYC {kycTheme.label}
              </span>
              <span className="px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-400 text-[10px] font-black tracking-widest uppercase">
                TRC20
              </span>
            </div>
          </div>
          ...
          <button
            ...
            className="shrink-0 w-full md:w-auto px-8 py-3.5 rounded-2xl bg-white text-slate-950 font-black italic uppercase hover:bg-cyan-400 transition-colors shadow-xl shadow-white/5 flex items-center justify-center gap-2 text-sm"
          >
      ...
      <motion.section
        ...
        className={`relative rounded-[40px] overflow-hidden border shadow-2xl ${
          isKycVerified      ? "border-emerald-500/30 shadow-emerald-500/10"
          : isProcessing     ? "border-blue-500/30 shadow-blue-500/10"
          : isKycPending     ? "border-amber-500/30 shadow-amber-500/10"
          : maxAttemptsReached ? "border-red-500/30 shadow-red-500/10"
          : isKycRejected    ? "border-rose-500/30 shadow-rose-500/10"
          :                    "border-cyan-500/20 shadow-cyan-500/10"
        }`}
      >
        {/* gradient bg */}
        <div className={`absolute inset-0 ${
          isKycVerified      ? "bg-gradient-to-br from-emerald-950/80 via-slate-900 to-slate-950"
          : isProcessing     ? "bg-gradient-to-br from-blue-950/80 via-slate-900 to-slate-950"
          : isKycPending     ? "bg-gradient-to-br from-amber-950/60 via-slate-900 to-slate-950"
          : maxAttemptsReached ? "bg-gradient-to-br from-red-950/60 via-slate-900 to-slate-950"
          : isKycRejected    ? "bg-gradient-to-br from-rose-950/60 via-slate-900 to-slate-950"
          :                    "bg-gradient-to-br from-cyan-950/40 via-slate-900 to-slate-950"
        }`} />
        {/* decorative shield */}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-8">
            ...
            <div className="flex items-start gap-5">
              <div className={`w-16 h-16 rounded-[24px] flex items-center justify-center shrink-0 shadow-xl ${
                isKycVerified      ? "bg-emerald-500/20 text-emerald-400 shadow-emerald-500/20"
                : isProcessing     ? "bg-blue-500/20 text-blue-400 shadow-blue-500/20"
                : isKycPending     ? "bg-amber-500/20 text-amber-400 shadow-amber-500/20"
                : maxAttemptsReached ? "bg-red-500/20 text-red-400 shadow-red-500/20"
                : isKycRejected    ? "bg-rose-500/20 text-rose-400 shadow-rose-500/20"
              ...
              <div className="space-y-1">
                <p className="text-slate-500 text-[10px] font-black tracking-widest uppercase">Identity Verification</p>
                <h2 className={`text-2xl font-black italic tracking-tighter uppercase ${
                  isKycVerified      ? "text-emerald-400"
                  : isProcessing     ? "text-blue-400"
                  : isKycPending     ? "text-amber-400"
                  : maxAttemptsReached ? "text-red-400"
                  : isKycRejected    ? "text-rose-400"
                  :                    "text-cyan-400"
                }`}>
            ...
            <div className="lg:ml-auto flex flex-col sm:flex-row lg:flex-col items-stretch sm:items-center lg:items-end gap-4 shrink-0">
              ...
              {/* Attempts tracker */}
              {!isKycVerified && (
                <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 min-w-[180px]">
                  <p className="text-slate-500 text-[9px] font-black tracking-widest uppercase mb-3">Verification Attempts</p>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400 text-xs font-bold">{kycAttempts} used</span>
                    <span className={`text-xs font-black ${attemptsLeft > 0 ? "text-white" : "text-red-400"}`}>
                      {attemptsLeft} left
                    </span>
                  </div>
                </div>
              ...
              {isKycVerified && (
                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-5 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-emerald-400 font-black text-sm uppercase italic">Full Access</p>
                    <p className="text-slate-400 text-[10px] mt-0.5">No restrictions</p>
                  </div>
                </div>
              )}
              ...
              {maxAttemptsReached && (
                <Link
                  to="/help-center"
                  className="px-8 py-4 rounded-2xl bg-slate-800 border border-slate-700 text-slate-300 font-black text-sm uppercase tracking-tight hover:border-slate-600 transition-all flex items-center justify-center gap-2"
                >
                ...
                {[
                  ...
                  <div key={s.id} className="flex items-center flex-1">
                    <div className="flex flex-col items-center gap-1.5 flex-1">
                      ...
                      <span className={`text-[9px] font-black tracking-widest uppercase text-center whitespace-nowrap ${s.done ? "text-cyan-400" : "text-slate-600"}`}>
                        {s.label}
                      </span>
                    </div>
                    {i < arr.length - 1 && (
                      <div className={`h-0.5 flex-1 mx-1 mb-4 rounded-full transition-all ${s.done ? "bg-cyan-500/50" : "bg-slate-800"}`} />
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {[
              ...
              <div key={label} className={`bg-slate-900/50 p-6 rounded-[32px] border border-slate-800/50 flex items-center justify-between group hover:border-${color}-500/30 transition-all`}>
                <div>
                  <p className="text-slate-500 text-[9px] font-black tracking-widest uppercase mb-1">{label}</p>
                  <h4 className="text-2xl font-black text-white italic tracking-tighter">{value}</h4>
                  <p className={`text-${color}-400 text-[9px] font-black tracking-widest uppercase mt-2 flex items-center gap-1`}>
                    <Icon className="w-3 h-3" /> {sub}
                  </p>
                </div>
                <div className={`w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center text-slate-500 group-hover:bg-${color}-500/10 group-hover:text-${color}-400 transition-all shrink-0`}>
          ...
          <section className="bg-slate-900/50 p-8 rounded-[40px] border border-slate-800/50">
            <h3 className="text-lg font-black text-white italic tracking-tighter uppercase mb-6 flex items-center gap-3">
              <Users className="w-5 h-5 text-indigo-400" /> Organization Dynamics
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Team",    val: referralData?.total_referrals || 0, color: "text-white"    },
                { label: "Direct Tier 1", val: referralData?.tree?.tier1 || 0,    color: "text-cyan-400"  },
                { label: "Tier 2",        val: referralData?.tree?.tier2 || 0,    color: "text-indigo-400"},
                { label: "Tier 3",        val: referralData?.tree?.tier3 || 0,    color: "text-blue-400"  },
              ].map(({ label, val, color }) => (
                <div key={label} className="p-5 bg-slate-800/20 rounded-3xl border border-slate-700/20 text-center">
                  <p className="text-slate-500 text-[9px] font-black tracking-widest uppercase mb-1">{label}</p>
        ...
        <div className="lg:col-span-4 space-y-5">
          <section className="bg-slate-900/50 p-5 rounded-[32px] border border-slate-800/50 space-y-1">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-3 pb-2">Management</p>
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-3 rounded-2xl transition-all group ${canStartKyc ? "hover:bg-cyan-500/10 cursor-pointer" : "cursor-default"}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all text-sm
                  ${isKycVerified ? "bg-emerald-500/10 text-emerald-400" : isKycPending ? "bg-amber-500/10 text-amber-400" : isKycRejected ? "bg-rose-500/10 text-rose-400" : "bg-slate-800 text-slate-400 group-hover:bg-cyan-500/10 group-hover:text-cyan-400"}`}>
                  <ShieldCheck className="w-4 h-4" />
                </div>
                <div className="text-left">
                  <p className="text-white font-bold text-sm tracking-tight uppercase leading-none">KYC Status</p>
                  <p className={`text-[9px] font-black tracking-widest uppercase mt-0.5 ${
                    isKycVerified ? "text-emerald-400" : isKycPending ? "text-amber-400" : maxAttemptsReached ? "text-red-400" : isKycRejected ? "text-rose-400" : "text-slate-500"
                  }`}>
          ...
          <section className="bg-slate-900/50 p-5 rounded-[32px] border border-slate-800/50 space-y-1">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-3 pb-2">Support & Legal</p>
            <NavRow to="/faq"   Icon={HelpCircle} label="Help Center"       hoverColor="white" external />
            <NavRow to="/legal" Icon={FileText}   label="Compliance & Legal" hoverColor="white" external />
            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-between p-3 rounded-2xl hover:bg-rose-500/10 transition-all group"
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-slate-800 flex items-center justify-center text-rose-400 group-hover:bg-rose-500 group-hover:text-white transition-all">
                  <LogOut className="w-4 h-4" />
                </div>
                <span className="text-rose-400 font-black text-sm tracking-tight uppercase">Secure Logout</span>
              </div>
...
function NavRow({
  ...
  const inner = (
    <div className="flex items-center gap-3">
      <div className={`w-9 h-9 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-${hoverColor}-400 group-hover:bg-${hoverColor}-500/10 transition-all`}>
        <Icon className="w-4 h-4" />
      </div>
      <span className="text-slate-300 font-bold text-sm tracking-tight uppercase">{label}</span>
    </div>
  );
  const cls = "w-full flex items-center justify-between p-3 rounded-2xl hover:bg-slate-800/50 transition-all group";
  if (to) return (
    <Link to={to} className={cls}>
      {inner}
      {external ? <ExternalLink className="w-3.5 h-3.5 text-slate-600" /> : <ChevronRight className="w-4 h-4 text-slate-700" />}
    </Link>
  );
  return (
